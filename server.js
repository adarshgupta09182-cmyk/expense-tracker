...
const server = app.listen(PORT, () => {
  ...
  console.log(`Mode: JSON File Storage (Legacy)`);
...
app.post('/api/admin/register', [
  ...
  try {
    ...
    if (adminSecret !== process.env.ADMIN_SECRET || !process.env.ADMIN_SECRET) {
      logger.warn('Admin registration failed: invalid admin secret', { email });
    ...
    if (users.find(u => u.role === 'admin')) {
      logger.warn('Admin registration failed: admin already exists', { email });
    ...
    logger.info('Admin registration successful', { email, adminId: admin.id });
...
app.post('/api/admin/login', [
  ...
  try {
    ...
    logger.info('Admin login successful', { email, adminId: user.id });
...
app.get('/api/admin/users/:userId', authenticate, authorize('admin'), (req, res) => {
  try {
    const users = JSON.parse(fs.readFileSync(USERS_FILE, 'utf8'));
    const expenses = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
...
app.delete('/api/admin/users/:userId', authenticate, authorize('admin'), (req, res) => {
  try {
    ...
    fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2));
    fs.writeFileSync(DATA_FILE, JSON.stringify(filteredExpenses, null, 2));

    logger.info('User deleted by admin', { userId, userName, adminId: req.user.id });

    res.json({
      success: true,
      message: `User ${userName} and all their expenses have been deleted`
    });
  } catch (error) {
...
// Delete user expense (admin only)
app.delete('/api/admin/expenses/:expenseId', authenticate, authorize('admin'), (req, res) => {
  try {
    const expenses = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
    const expenseIndex = expenses.findIndex(e => e._id === req.params.expenseId);
    ...
    logger.info('Expense deleted by admin', { expenseId: req.params.expenseId, adminId: req.user.id });
